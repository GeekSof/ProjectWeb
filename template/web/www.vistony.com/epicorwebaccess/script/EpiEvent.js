...
function EpiEventDelayedCallback(func)
{ 
    this.Method = func;
}
EpiEventDelayedCallback.prototype.Dispose = function()
{ 
    if (typeof this.Method != "undefined")
        delete this.Method;
}

function EpiEvent(type, oScope) 
{
    this._type = "EpiEvent";
    this.type = type;
    this.scope = oScope || Global.window;
    this.subscribers = [];
    this.returnValue = true;
}

EpiEvent.prototype.subscribe = function(fn, obj, bOverride) 
{
    this.subscribers.push( new Subscriber(fn, obj, bOverride) );
}

EpiEvent.prototype.unsubscribe = function(fn, obj) 
{
    var found = false;
...
EpiEvent.prototype.fire = function()
{
    var retVal = true;
    var numSub = this.subscribers.length;
    var runAfterAll = [];
    for (var i=0; i<numSub; ++i) 
    {
        ...
        if (s) {
            ...
            try
            {
                ...
                var retnVal = s.fn.apply(scope,arguments); 
                if (retnVal instanceof EpiEventDelayedCallback)
                    runAfterAll[runAfterAll.length] = retnVal;

                if (retnVal != undefined) retVal = retnVal;
            ...
            catch(err)
            {
                ...
                catch(err2){ fnStr = "Can't determine."; }
                
                DebugHelper.WriteError("EpiEvent.fire error for '" + this.type + "'.  Function text is: <br />" + fnStr + "<br/><br/>", err);
                retVal = false;
                
                if(Global.InstanceOf(err,"UIException"))
                {
                    throw err;
                }
            }
    ...
    for (var i = 0; i < runAfterAll.length; i++)
    {
        runAfterAll[i].Method();
        runAfterAll[i].Dispose();
        runAfterAll[i] = null;
    }
...
EpiEvent.prototype.unsubscribeAll =  function()
{
    for (var i=0; i<this.subscribers.length; ++i) {
        this._delete(i);
    }
...
EpiEvent.prototype._delete = function(index)
{
...
function Subscriber(fn, obj, bOverride) 
{
    this.fn = fn;
    this.obj = obj || null;
...
Subscriber.prototype.contains = function(fn, obj) 
{
    ...
    return { // PREPROCESS
        ...
        addListener: function(el, sType, fn, oScope, bOverride) {
            ...
            var wrappedFn = function(e) {
                    return fn.call(scope, EpiEventManager.getEvent(e), 
                            oScope);
                };
            ...
            if (this.useLegacyEvent(el, sType)) {
                ...
                if (legacyIndex == -1) {
                    ...
                    el["on" + sType] = 
                        function(e) {
                            EpiEventManager.fireLegacyEvent(
                                EpiEventManager.getEvent(e), legacyIndex);
                        };
                }
            ...
            {
                if(sType=="focus" && BrowserSniffer.FireFox15)
                    el.addEventListener(sType, wrappedFn, true);
                else
        ...
        /**
         * Shorthand for EpiEventManager.addListener
         * @type function
         */
        // on: this.addListener,
        ...
        useLegacyEvent: function(el, sType) {
            ...
            if (!el.addEventListener && !el.attachEvent) {
               ...
               /*
				* this events work fine in Safari now.
        ...
        _unload: function(e, me) {
            for (var i=0,len=unloadListeners.length; i<len; ++i) {
                ...
                if (l) {
                    var scope = (l[this.ADJ_SCOPE]) ? l[this.SCOPE]: Global.window;
         ...
         */
        _getScroll: function() {
            var dd = Global.document.documentElement; db = Global.document.body;
            if (dd && dd.scrollTop) {
 ...
 * @private
 */
EpiEventManager.on = EpiEventManager.addListener;

if (document && document.body) {
    EpiEventManager._load();
} else {
    EpiEventManager.on(window, "load", EpiEventManager._load, 
            EpiEventManager, true);
}

EpiEventManager.on(window, "unload", EpiEventManager._unload, 
            EpiEventManager, true);

// shouldn't need this, because we don't attach before form load.  
//EpiEventManager._tryPreloadAttach();
